...
clean:
	rm -f bin/*~ $(PYTHONC_FILES)
	rm -f bin/*~

translate_get:
	xgettext -k_ -kN_ -o bin/po/terp-msg.pot $(PYTHON_FILES)

translate_set2:
	msgfmt bin/po/fr.po -o bin/po/fr/LC_MESSAGES/terp.mo

translate_set:
	msgfmt bin/po/ca.po -o bin/po/ca/LC_MESSAGES/terp.mo
...
merge:
	msgmerge bin/po/ca.po bin/po/terp-msg.pot -o bin/po/ca.po
