...
mapIcons = {
        ...
        'terp-camera_test': 'ok.png',
        'terp-check' : 'ok.png',
        'terp-crm' : 'crm.png',
        'terp-dialog-close': 'close.png',
        'terp-document-new' : 'new.png',
        'terp-go-today' : 'go-today.png',
        'terp-graph' : 'chart.png',
        'terp-gtk-go-back-ltr' : 'previous.png',
        'terp-gtk-go-back-rtl' : 'reload.png',
        'terp-gtk-jump-to-ltr' : 'next.png',
        'terp-gtk-jump-to-rtl' : 'previous.png',
        'terp-gtk-media-pause' : 'pause.png',
        'terp-hr' : 'hr.png',
        'terp-mrp' : 'mrp.png',
...
def kdePixmap(icon):
        """ Returns a QPixmap given an icon name.

            The name of the icon is usually a GTK or OpenERP name.
        """
        global _pixmapCache
        ...
        if icon not in _pixmapCache:
            log.debug('Attempting to load "%s" from image resources', icon)
            # TODO: move mapping to resource file
            # TODO: lazy-loading resources from OpenERP server
            pix = None
            if icon in mapIcons:
                pix = QPixmap( ':/images/' + mapIcons[icon] )
                # if pix.isNull():
                #       log.debug("Image file %s not found",...)
            else:
                log.warning("KDE icon '%s' not found", icon)
                pix = QPixmap()
            _pixmapCache[icon] = pix

        return _pixmapCache[icon]

def kdeIcon(icon):
        """ Returns a QIcon given an icon name.

            The name of the icon is usually a GTK or OpenERP name.
        """
        return QIcon(kdePixmap(icon))
