...
class fulltextsearch_services(netsvc.Service):
	...
	def search(self, db, uid, passwd, text, limit, offset, model, context={}):
		...
		for x in text.split(u' '):
			if isFloat(x):
				tsVector.append( u"to_tsvector( 'default', %f::TEXT )" % float(x) )
				tsQuery.append( u"to_tsquery( 'default', %f::TEXT )" % float(x) )
			elif isInteger(x):
				tsVector.append( u"to_tsvector( 'default', %d::TEXT )" % long(x) )
				tsQuery.append( u"to_tsquery( 'default', %d::TEXT )" % long(x) )
			else:
		...
		if model:
			filterModel = u' AND m.id = %d ' % int(model)
		else:
			filterModel = u''
